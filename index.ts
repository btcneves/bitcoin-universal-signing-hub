...
const decodeBase58Check = (value: string): Buffer | undefined => {
    ...
    const firstHash = CryptoJS.SHA256(CryptoJS.enc.Hex.parse(payload.toString('hex'))).toString();
    const secondHash = CryptoJS.SHA256(firstHash).toString();
    const digest = Buffer.from(secondHash, 'hex');
...
export class UniversalQrService implements QRService {
    detectPayload(input: string): ParsedQRPayload {
        ...
        if (bech.ok || base58.ok) {
            return { type: 'bitcoin_address', raw: trimmed, metadata: { network: bech.network ?? base58.network ?? 'mainnet' } };
        }
